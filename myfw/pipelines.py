...
class MyfwPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('myfw.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `myfw`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM myfw WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO myfw (date, title, link, content)"
